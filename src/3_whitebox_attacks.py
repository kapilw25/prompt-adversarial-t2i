...
class SoftEmbeddingAttacker:
    ...
    def optimize_soft_embeddings(self, 
        ...
        for iteration in range(max_iterations):
            ...
            try:
                ...
                if score > best_score:
                    ...
                    # Save best image
                    if generated_images:
                        output_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), "outputs", "step3_whitebox")
                        ...
                        best_image_path = filename  # Track the best image path
        ...
        if db is not None and best_result is not None:
            # Store whitebox attack - let database generate deterministic ID
            attack_id = db.store_whitebox_attack(
                ...
                best_image_path=best_image_path,  # Use the tracked best image path
                total_iterations=max_iterations,
                final_embeddings=soft_embeddings,
                stealth_metrics=best_result.get("stealth_metrics", {}),
                attack_success=best_result["score"] > 0.7
            )
            
            # Store iterations
            for i, (prompt, clip_loss, div_loss, stealth_loss) in enumerate(zip(
                prompt_history, clip_loss_history, diversity_loss_history, stealth_loss_history
            )):
                db.store_whitebox_iteration(
                    attack_id=attack_id,
                    iteration_num=i + 1,
                    prompt_text=prompt,
                    clip_loss=clip_loss,
                    ...
                    total_loss=clip_loss + div_loss + stealth_loss
                )
...
def main():
    """Demo white-box attack with database storage"""
    print("="*60)
    ...
    for scenario in attack_scenarios:
        ...
        scenario_name = f"whitebox_{attack_type}"
        
        cursor.execute("SELECT attack_id FROM step3_whitebox_attacks WHERE scenario_name = ?", (scenario_name,))
    ...
    for i, scenario in enumerate(scenarios_to_process):
        ...
        # Plot optimization progress in outputs directory
        plots_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), "outputs", "step3_whitebox", "plots")
        ...
        print(f"Optimization progress plot saved: {plot_filename}")
    ...
    print(f"Database contains {len(whitebox_attacks)} white-box attacks:")

    attack_ids = []
    for attack in whitebox_attacks[-len(attack_scenarios):]:  # Show latest attacks from this run
    ...
    for attack_id in attack_ids:
        iterations = db.get_whitebox_iterations(attack_id)
